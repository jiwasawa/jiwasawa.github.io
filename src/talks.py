...
def presentation_item(title, title_jp, venue, date, location="", venue_link=""):
    ...
    venue_node = A(venue, href=venue_link, cls=AT.primary) if venue_link else venue
    parts = [Strong(title)]
    if title_jp:
        parts += [Br(), title_jp]
    parts += [Br(), Em(venue_node), f", {date}{location_text}."]
    return Li(*parts)
        ...
        Section(
            ...
            Ul(
                ...
                presentation_item(
                    ...
                    "Finetuning or RAG? 医療ドメイン特化 LLM 構築に向けて",
